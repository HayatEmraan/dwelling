...
const InvoiceListPage = () => {
  return (
    ...
    {
        dashboardCards.map(({id, title, icon,clientPercentage, amount,arrow,bgColor})=>{
            return (
                <div key={id} className="shadow-md p-4 w-full rounded-md bg-white transition-all hover:-translate-y-1">
                    ...
                    {/* <span className="text-2xl rounded-md">
                            <div className={`${icon === <FiUsers /> ? ' bg-[#e1c38f] text-[#706f6e] ': icon === <BsFileEarmarkText /> ? ' bg-[#a5f3fc] text-[#0891b2]' : icon === <GiSelfLove/> ? 'bg-[#fecdd3] text-[#f43f5e]' : icon === <SiPayoneer />? 'bg-[#e9d5ff] text-[#a855f7 ' :  ''}`}> 
                            {icon}</div>
...
<span className="text-2xl rounded-md ">
    <div className={`p-2 rounded ${ title ==='Clients'? `bg-[${bgColor}] text-[#706f6e]` : 
                    title === 'Invoices' ? 'bg-[#a5f3fc] text-[#0891b2]' : 
                    title === 'Sent Invoice' ? 'bg-[#fecdd3] text-[#f43f5e]' : 
                    title === 'Paid' ? 'bg-[#e9d5ff] text-[#a855f7]' : 
                    ''}` }>
        {icon}
    </div>
</span>
...
<span>
                     ...
                     <span className="rounded-md text-[#5d91eb] -2 flex">
                        ...
                        <div className={` flex  ${clientPercentage === '+9.18' ? 'text-green-500': clientPercentage ==='+183' ? ' text-green-500' :clientPercentage ==='+6.18' ? 'text-green-500' : clientPercentage === '-3.18' ? 'text-red-500 ' :  ''}`}> <div className="mt-1">{arrow} </div>   {clientPercentage}%</div>
